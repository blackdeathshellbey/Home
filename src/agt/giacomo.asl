...
+!show_winners
   <- for (currentWinner(Ag)[artifact_id(ArtId)]) {
         ...
         // Trigger the common plan for processing the winner belief
         !process_winner_belief[source(Ag)];
      }.

+!process_winner_belief
   <- ?believer(Agent)[ +winner(won(Task)) ];
      println("Received belief: winner(won(", Task, ")) from agent ", Agent);
      assert(winner_info(Agent, Task));
      println("Agent ", Agent, " won task ", Task);
      rd(winner_info(Agent, Task));
      println("Read tuple: ", Agent, " won task ", Task);
      // Trigger the common plan for processing the winner tell message
      !won(Agent, Task).
