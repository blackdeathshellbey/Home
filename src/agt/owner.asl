...
+!show_winners
   <- for (currentWinner(Ag)[artifact_id(ArtId)]) {
         ...
         // Send the tell message to the winner
         .tell(Ag, winner(won(Task)));
      }
   ;
...
+!execute
   <- println;
      ...
      println("Waiting for the `go` from the user");
      //!!go;  // Commented to stop the execution and waiting for a message
   .
